/* ============================================================
   ...
   • Barometer (Capacitor Native / Browser APIs)
...
import { CapacitorBarometer as CapBarometer } from '@capgo/capacitor-barometer';
import pkg from '../package.json';

if (typeof ethers === 'undefined') {
  alert('❌ Error: La librería ethers.js no se cargó. Revisá tu conexión o el CDN.');
}
...
function onPressureReading(hpa, tempC) {
  ...
  if (state.altitudePrev !== null) {
    ...
    // clamp noise (max 20m/s delta between readings as outlier)
...
async function startSensor() {
  console.log('Attempting to start barometer...');
  const isNative = window.Capacitor && window.Capacitor.isNativePlatform();
  const isAndroidBrowser = /Android/i.test(navigator.userAgent) && !isNative;

  // 0. Try Capacitor Native Barometer (Strongest bypass)
  try {
    if (isNative) {
      console.log('Running on native Capacitor platform');
      renderLog('✅ Capacitor Bridge detectado');
      const check = await CapBarometer.isAvailable();
      if (check.isAvailable) {
        await CapBarometer.addListener('measurement', (data) => {
          onPressureReading(data.pressure);
        });
        await CapBarometer.startMeasurementUpdates();
        state.sensorType = 'capacitor';
        setStatus('active', 'Barómetro Nativo (Capacitor)');
        if (sensorTypeEl) sensorTypeEl.textContent = 'CAPACITOR NATIVE API';
        renderLog('Sensor: Capacitor Native Barometer ✅');
        return;
      } else {
        renderLog('🔸 CapBarometer reporta sensor no disponible');
      }
    } else {
      console.log('Not on native Capacitor platform, trying browser APIs...');
      if (isAndroidBrowser) {
        renderLog('📱 Browser Android detectado — probando APIs de sensor…');
      }
    }
  } catch(e) { 
    console.log('Capacitor native check failed or not in app:', e);
    renderLog('❌ Capacitor check: ' + e.message);
  }

  // 1. Check Permissions API for sensors
  if (navigator.permissions && navigator.permissions.query) {
    try {
      const p = await navigator.permissions.query({ name: 'ambient-pressure' });
      console.log('Permission pressure:', p.state);
      renderLog('🔑 Permiso sensor: ' + p.state);
    } catch(e) { console.log('Permission query fail (normal en la mayoría de browsers):', e); }
  }

  // 2. Try AmbientPressureSensor (Generic Sensor API - Chrome/Android standard)
  if ('AmbientPressureSensor' in window) {
    try {
      sensorInst = new AmbientPressureSensor({ frequency: 10 });
      sensorInst.addEventListener('reading', () => {
        onPressureReading(sensorInst.pressure, undefined);
      });
      sensorInst.addEventListener('error', (e) => {
        console.warn('AmbientPressureSensor err:', e);
        if (e.error.name === 'NotAllowedError') toast('❌ Acceso denegado al sensor');
        else if (e.error.name === 'NotReadableError') toast('❌ Sensor no responde (hardware)');
      });
      sensorInst.start();
      state.sensorType = 'AmbientPressure';
      setStatus('active', 'Barómetro Nativo');
      if (sensorTypeEl) sensorTypeEl.textContent = 'AMBIENT PRESSURE API';
      renderLog('Sensor: AmbientPressure (Nativo) ✅');
      return;
    } catch (e) { 
      console.warn('AmbientPressureSensor fail:', e);
      renderLog('❌ AmbientPressureSensor: ' + e.message);
    }
  }

  // 3. Try PressureSensor (Alias used in some implementations)
  if ('PressureSensor' in window) {
    try {
      sensorInst = new PressureSensor({ frequency: 10 });
      sensorInst.addEventListener('reading', () => {
        onPressureReading(sensorInst.pressure);
      });
      sensorInst.start();
      state.sensorType = 'PressureSensor';
      setStatus('active', 'Sensor Presión');
      if (sensorTypeEl) sensorTypeEl.textContent = 'PRESSURE SENSOR API';
      renderLog('Sensor: PressureSensor API ✅');
      return;
    } catch (e) {
      console.warn('PressureSensor fail:', e);
      renderLog('❌ PressureSensor: ' + e.message);
    }
  }

  // 4. Try Barometer (Samsung / Chrome experimental)
  if ('Barometer' in window) {
    try {
      sensorInst = new window.Barometer({ frequency: 10 });
      sensorInst.addEventListener('reading', () => {
        // Some older implementations return pressure in Pascals, some in hPa.
        // Usually it's in hPa if it follows the latest spec, but old was Pascals.
        // Let's assume hPa and if it's > 2000 it's probably Pascals.
        let p = sensorInst.pressure;
        if (p > 5000) p /= 100; // Pa to hPa
        onPressureReading(p);
      });
      sensorInst.start();
      state.sensorType = 'Barometer';
      setStatus('active', 'Barómetro Samsung');
      if (sensorTypeEl) sensorTypeEl.textContent = 'BAROMETER API';
      renderLog('Sensor: Barometer (Samsung/Generic) ✅');
      return;
    } catch (e) {
      console.warn('Barometer fail:', e);
      renderLog('❌ Barometer: ' + e.message);
    }
  }

  // 5. Fallback: Geolocation altitude
  if ('geolocation' in navigator) {
    try {
      ...
      if (pos.coords.altitude !== null) {
        ...
        setStatus('active', 'Altitud GPS');
        if (sensorTypeEl) sensorTypeEl.textContent = 'GPS ALTITUDE (FALLBACK)';
        renderLog('Sensor: GPS Altitud (Fallback)');
        ...
        sensorInterval = setInterval(() => {
          navigator.geolocation.getCurrentPosition(updateGPS, ()=>{} , { enableHighAccuracy: true, timeout: 2000 });
        }, 1500);
  ...
  // 6. No sensor found — show helpful guidance
  if (isAndroidBrowser) {
    activateSimMode(true);
  } else {
    activateSimMode(false);
  }
}

function activateSimMode(isAndroidBrowser = false) {
  state.sensorType = 'sim';
  setStatus('active', 'Simulación');
  if (sensorTypeEl) sensorTypeEl.textContent = 'MODO SIMULACIÓN (SIN SENSOR)';
  
  if (isAndroidBrowser) {
    toast('⚠ Barómetro no accesible desde browser', 5000);
    renderLog('────────────────────────────');
    renderLog('💡 Para activar el barómetro desde Chrome:');
    renderLog('   1. Abrí chrome://flags');
    renderLog('   2. Buscá "Generic Sensor Extra Classes"');
    renderLog('   3. Activalo y reiniciá Chrome');
    renderLog('   ─ O ─');
    renderLog('   Instalá el APK nativo (Android Studio)');
    renderLog('────────────────────────────');
  } else {
    toast('⚠ Sensor no detectado');
  }
...
function stopSensor() {
  if (state.sensorType === 'capacitor') {
    try { CapBarometer.stopMeasurementUpdates(); } catch(e){}
  }
...
async function deriveKey(hex) {
  // Ensure the hex string has the 0x prefix for ethers.getBytes
  const h = (typeof hex === 'string' && !hex.startsWith('0x')) ? '0x' + hex : hex;
  const raw = ethers.getBytes(h);
...
const scanSensorsBtn = document.getElementById('scan-sensors-btn');

scanSensorsBtn.addEventListener('click', async () => {
  renderLog('--- Iniciando Escaneo ---');
  
  if (window.Capacitor && window.Capacitor.isNativePlatform()) {
    renderLog(`✅ Capacitor Bridge detectado (Plataforma: ${window.Capacitor.getPlatform()})`);
    try {
      const check = await CapBarometer.isAvailable();
      renderLog(`✅ CapBarometer disponible (Nativo): ${check.isAvailable}`);
    } catch(e) { renderLog('❌ CapBarometer error: ' + e.message); }
  } else {
    renderLog('🔸 Capacitor (Nativo) no disponible en navegador');
    renderLog('🔸 Usa la app de Android Studio para probar CapBarometer');
  }
  ...
  apis.forEach(api => {
    if (api in window) {
      renderLog(`✅ ${api} detectada`);
      found++;
    }
  });

  // Check generic 'Sensor' prefix
  for (let key in window) {
    if (key.includes('Sensor') && !apis.includes(key)) {
      renderLog(`🔸 ${key} disponible`);
      found++;
    }
  }

  if (found === 0) renderLog('❌ Ningún sensor detectado');
  else renderLog(`Escaneo completo: ${found} ítems`);
});

const INSTALL_DISMISSED_KEY = 'vario_install_dismissed';
...
if (downloadApkBtn) {
  downloadApkBtn.addEventListener('click', () => {
    window.location.href = 'https://github.com/xunorus/variofly/releases/latest/download/vario-release.apk';
    hideInstallBanner(true);
  });
}

/** Is running inside installed PWA? */
...
/** Is iOS Safari (no beforeinstallprompt support) */
function isIosSafari() {
  const ua = navigator.userAgent;
  return /iPad|iPhone|iPod/.test(ua) && !window.MSStream && /Safari/.test(ua) && !/CriOS|FxiOS/.test(ua);
}

// Capture the install prompt event ASAP — must be done synchronously
window.addEventListener('beforeinstallprompt', (e) => {
  e.preventDefault();
  deferredInstallPrompt = e;
  // Show banner after short delay so UI is rendered
  if (!isInstalledPWA() && !sessionStorage.getItem(INSTALL_DISMISSED_KEY)) {
    setTimeout(showInstallBanner, 1800);
  }
});

// App was installed (Chrome fires this)
window.addEventListener('appinstalled', () => {
  hideInstallBanner();
  deferredInstallPrompt = null;
  toast('✅ VARIO instalada correctamente');
});
...
function hideInstallBanner(permanent = false) {
  ...
  if (permanent) sessionStorage.setItem(INSTALL_DISMISSED_KEY, '1');
}
...
installConfirmBtn.addEventListener('click', async () => {
  ...
  if (outcome === 'accepted') {
    toast('🚀 Instalando VARIO…');
  } else {
    toast('Podés instalarla más tarde desde el menú del browser');
  }
  deferredInstallPrompt = null;
});

installDismissBtn.addEventListener('click', () => hideInstallBanner(true));

// iOS: show hint if not installed and not dismissed
function maybeShowIosHint() {
  if (!isIosSafari() || isInstalledPWA()) return;
...
(async () => {
  ...
  // Set version
  const vEl = document.getElementById('app-v');
  if (vEl) vEl.textContent = 'v' + pkg.version;
